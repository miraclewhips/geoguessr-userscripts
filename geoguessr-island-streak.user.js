...
const save = () => {
	window.localStorage.setItem('geoIslandStreak', JSON.stringify(DATA));
}
...
const createStreakText = () => {
	...
	if(DATA.streak > 0) {
		return `It was <span style="color:#6cb928">${DATA.island_location}!</span> Island Streak: <span style="color:#fecd19">${DATA.streak}</span>`;
	}else{
		let suffix = `islands in a row.`;
		...
		if(DATA.island_guess) {
			previousGuessText = `You guessed <span style="color:#f95252">${DATA.island_guess}</span>, unfortunately it was <span style="color:#6cb928">${DATA.island_location}</span>.`;
		}
...
const stopRound = async () => {
	...
	if(responseGeoGuessr.player.guesses[guess_counter-1].timedOut && !responseGeoGuessr.player.guesses[guess_counter-1].timedOutWithGuess) {
		DATA.checking_api = false;
		DATA.island_guess = null;
		DATA.island_location = null;
		updateStreak(0);
		return;
	}
	...
	DATA.island_guess = getIsland(responseGuess);
	DATA.island_location = getIsland(responseLocation);

	if (guessCC && locationCC && guessCC === locationCC && DATA.island_guess === DATA.island_location) {
