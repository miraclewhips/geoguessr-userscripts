...
const createStreakText = () => {
	...
	if(DATA.streak > 0) {
		return `Country Streak: ${DATA.streak}`;
	}else{
		switch(DATA.previous_streak) {
			case 1:
				return `Your streak ended after correctly guessing ${DATA.previous_streak} country.`;
			default:
				return `Your streak ended after correctly guessing ${DATA.previous_streak} countries in a row.`;
		}
	}
}
...
const queryAPI = async (location) => {
	...
	let apiUrl = `https://nominatim.openstreetmap.org/reverse.php?lat=${location[0]}&lon=${location[1]}&zoom=3&format=jsonv2`;
